...
from .forms import ChangeUserInfoForm, RegisterUserForm, BbForm, AIFormSet, UserCommentForm, GuestCommentForm
...
class BBPasswordChangeView(LoginRequiredMixin, SuccessMessageMixin, PasswordChangeView):
    ...
    success_massage = 'Пароль успешно изменен'
...
def profile_bb_change(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            bb = form.save()
            fromset = AIFormSet(request.POST, request.FILES, instance=bb)
            if fromset.is_valid():
                fromset.save()
        ...
        fromset = AIFormSet(instance=bb)
    context = {'form': form, 'formset': fromset}
